  ...
  module Utility
    ...
    module NativeInteger
      ...
      def ensure_upper_bound(value)
        if value > MAX_VALUE
          raise RangeError, "#{value} is greater than the maximum value of #{MAX_VALUE}"
        end
        value
      end
      ...
      def ensure_lower_bound(value)
        if value < MIN_VALUE
          raise RangeError, "#{value} is less than the maximum value of #{MIN_VALUE}"
        end
        value
      end
      ...
      def ensure_positive_and_no_zero(value)
        if value < 1
          raise ArgumentError, "#{value} cannot be negative or zero"
        end
        value
      end
